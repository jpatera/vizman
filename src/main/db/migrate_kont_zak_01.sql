...
-- TRUNCATE TABLE ZAVIN.CAL_;
-- TRUNCATE TABLE ZAVIN.CIN_;
-- TRUNCATE TABLE ZAVIN.CONFIG;
-- TRUNCATE TABLE ZAVIN.DOCH_;
-- TRUNCATE TABLE ZAVIN.DOCH_SUM;
-- TRUNCATE TABLE ZAVIN.DOKLADY;
-- TRUNCATE TABLE ZAVIN.EXT_;
-- TRUNCATE TABLE ZAVIN.SVATKY;
-- TRUNCATE TABLE ZAVIN.USR_;
-- TRUNCATE TABLE ZAVIN.USR_SAZBA;
-- TRUNCATE TABLE ZAVIN.UZAV_;
-- TRUNCATE TABLE ZAVIN.VYKONY_;
-- TRUNCATE TABLE ZAVIN.ZAK_;

-- =============================================

-- Kontrola pred ostrou 31. 3. 2019

-- 4687
SELECT COUNT(*) FROM ZAVIN.CAL_;

-- 13
SELECT COUNT(*) FROM ZAVIN.CIN_;

-- 1
SELECT COUNT(*) FROM ZAVIN.CONFIG;

-- 34082
SELECT COUNT(*) FROM ZAVIN.DOCH_;

-- 36579
SELECT COUNT(*) FROM ZAVIN.DOCH_SUM;

-- 1369
SELECT COUNT(*) FROM ZAVIN.DOKLADY;

-- 12
SELECT COUNT(*) FROM ZAVIN.EXT_;

-- 11
SELECT COUNT(*) FROM ZAVIN.SVATKY;

-- 43
SELECT COUNT(*) FROM ZAVIN.USR_;

-- 113
SELECT COUNT(*) FROM ZAVIN.USR_SAZBA;

-- 172
SELECT COUNT(*) FROM ZAVIN.UZAV_;
...
CREATE TABLE VIZMAN.CALY (
	...
	VERSION INTEGER NOT NULL,
	YR INTEGER NOT NULL,
	YEAR_FOND_HOURS DECIMAL(6,1),
	YEAR_FOND_DAYS DECIMAL(5,1),	
	CONSTRAINT PK_CALY PRIMARY KEY (ID)
);

CREATE UNIQUE INDEX VIZMAN.IDXQ_CALY_YR ON VIZMAN."CALY" (YR)
;
...
INSERT INTO VIZMAN.CALY (ID, VERSION, YR, YEAR_FOND_HOURS)
	SELECT DISTINCT 100 * ROK_UZAV, 0, ROK_UZAV, SUM(PRACFOND) FROM zavin.UZAV_
	GROUP BY ROK_UZAV
;

UPDATE VIZMAN.CALY
	SET YEAR_FOND_HOURS = 2008, YEAR_FOND_DAYS = 251
WHERE YR = 2019
;


UPDATE VIZMAN.CALY
	SET YEAR_FOND_DAYS = YEAR_FOND_HOURS / 8
;	

COMMIT;

SELECT * FROM VIZMAN.CALY
;
...
CREATE TABLE VIZMAN.CALYM (
	...
	VERSION INTEGER NOT NULL,
	YM INTEGER NOT NULL,
	MONTH_FOND_HOURS DECIMAL(6,1),
	MONTH_FOND_DAYS DECIMAL(5,1),
	CONSTRAINT PK_CALYM PRIMARY KEY (ID)
);

CREATE UNIQUE INDEX VIZMAN.IDXQ_CALYM_YM ON VIZMAN.CALYM (YM)
;
...
INSERT INTO VIZMAN.CALYM (ID, VERSION, YM, MONTH_FOND_HOURS)
	SELECT DISTINCT 100 * ROK_UZAV + MES_UZAV, 0, 100 * ROK_UZAV + MES_UZAV AS YM, SUM(PRACFOND)
	FROM zavin.UZAV_
	GROUP BY YM
	ORDER BY YM
;
...
INSERT INTO VIZMAN.CALYM (ID, VERSION, YM, MONTH_FOND_HOURS)
(
SELECT 100 * YEAR(ymd) + MONTH(ymd), 0,  100 * YEAR(ymd) + MONTH(ymd), fnd
FROM (
	SELECT CONVERT(CONCAT(@yr, '-1-1'), DATE) AS ymd, 176 AS fnd UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-2-1'), DATE), 160 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-3-1'), DATE), 168 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-4-1'), DATE), 160 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-5-1'), DATE), 168 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-6-1'), DATE), 160 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-7-1'), DATE), 176 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-8-1'), DATE), 176 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-9-1'), DATE), 168 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-10-1'), DATE), 176 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-11-1'), DATE), 168 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-12-1'), DATE), 152 
) yearmonth
)
ORDER BY ymd
;
...
UPDATE VIZMAN.CALYM
	SET MONTH_FOND_DAYS = MONTH_FOND_HOURS / 8
;	

COMMIT;

-- 180
SELECT count(*) FROM VIZMAN.CALYM
;
...
CREATE UNIQUE INDEX VIZMAN.IDXQ_CALYHOL_YR_HOLDATE ON VIZMAN.CALY_HOL (YR, HOL_DATE)
;

INSERT INTO VIZMAN.CALY_HOL
(ID, VERSION, YR, HOL_DATE, HOL_TEXT)
SELECT DISTINCT
...
INSERT INTO VIZMAN.CALY_HOL	(ID, VERSION, YR, HOL_DATE, HOL_TEXT)
...
-- 140
SELECT count(*) FROM VIZMAN.CALY_HOL
;
...
UPDATE VIZMAN.CALY
SET PRAC_FOND_YR =
	(SELECT SUM(MONTH_FOND) FROM VIZMAN.CALYM
		WHERE YM / 100 = 2019
	)
WHERE CAL_YR = 2019
;
...
-- 13
SELECT count(*) FROM VIZMAN.CIN
;


-- =================================================================================
--  Create tables - PERSON, ROLE, PERSON_ROLE, PERSON_WAGE, ROLE_PERM
-- =================================================================================
...
(ID, VERSION, STATE, HIDDEN, LOGIN_ENABLED, JMENO, PRIJMENI, PASSWORD, USERNAME, NASTUP, VYSTUP, SAZBA)
VALUES(1001, 0, 'LOGGEDOUT', TRUE, TRUE, 'Admin', 'Systémák', 'Admin3116', 'admin', NULL, NULL, 0);

INSERT INTO VIZMAN.PERSON
(ID, VERSION, STATE, HIDDEN, LOGIN_ENABLED, JMENO, PRIJMENI, PASSWORD, USERNAME, NASTUP, VYSTUP, SAZBA)
VALUES(1002, 0, 'LOGGEDOUT', TRUE, TRUE, 'User', 'Bìžný', 'user', 'user', NULL, NULL, 0);

INSERT INTO VIZMAN.PERSON
(ID, VERSION, STATE, HIDDEN, LOGIN_ENABLED, JMENO, PRIJMENI, PASSWORD, USERNAME, NASTUP, VYSTUP, SAZBA)
VALUES(1003, 0, 'LOGGEDOUT', TRUE, TRUE, 'Manager', 'Zkušený', 'Manager3116', 'manager', NULL, NULL, 0);

INSERT INTO VIZMAN.PERSON
	(ID, VERSION, STATE, HIDDEN, LOGIN_ENABLED,
	JMENO, PRIJMENI, PASSWORD, USERNAME, NASTUP, VYSTUP, SAZBA)
SELECT
	USER_ID, 1, 'LOGGEDOUT', FALSE, FALSE,
...
CREATE TABLE VIZMAN.PERSON_ROLE (
	...
	CONSTRAINT PK_PERSONROLE PRIMARY KEY (PERSON_ID,ROLE_ID),
	CONSTRAINT FK_PERSONROLE_ROLE FOREIGN KEY (ROLE_ID) REFERENCES VIZMAN."ROLE"(ID) ON DELETE RESTRICT ON UPDATE RESTRICT,
	CONSTRAINT FK_PERSONROLE_PERSON FOREIGN KEY (PERSON_ID) REFERENCES VIZMAN.PERSON(ID) ON DELETE RESTRICT ON UPDATE RESTRICT
);
...
CREATE TABLE VIZMAN.ROLE_PERM (
	ROLE_ID BIGINT NOT NULL,
	PERM VARCHAR(255),
	CONSTRAINT FK_ROLEPERM_ROLE FOREIGN KEY (ROLE_ID) REFERENCES VIZMAN."ROLE"(ID) ON DELETE RESTRICT ON UPDATE RESTRICT
);

CREATE INDEX VIZMAN.IDX_ROLEPERM_ROLEID
	ON VIZMAN.ROLE_PERM (ROLE_ID)
;
...
DROP TABLE VIZMAN.PERSON_WAGE IF EXISTS
;

CREATE TABLE VIZMAN.PERSON_WAGE (
	ID BIGINT NOT NULL,
	VERSION INTEGER NOT NULL,
	PERSON_ID BIGINT NOT NULL,
	USERNAME VARCHAR,
	YM_FROM INTEGER NOT NULL,
	YM_TO INTEGER,
	WAGE DECIMAL,
	CONSTRAINT PK_PERSONWAGE PRIMARY KEY (ID),
	CONSTRAINT FK_PERSONWAGE_PERSON FOREIGN KEY (PERSON_ID) REFERENCES VIZMAN.PERSON(ID) ON DELETE RESTRICT ON UPDATE RESTRICT
);
...
INSERT INTO VIZMAN.ROLE_PERM
(ROLE_ID, PERM)
VALUES(10003, 'PRUH_USE');

INSERT INTO VIZMAN.ROLE_PERM
(ROLE_ID, PERM)
VALUES(10003, 'VIEW_OTHER_USERS');
...
-----------------------------------------
--  Roles - INSERT to PERSON_ROLE
-----------------------------------------

--  Standard users
INSERT INTO VIZMAN.PERSON_ROLE
	(PERSON_ID, ROLE_ID)
SELECT DISTINCT id, 10002
FROM vizman.person
WHERE id < 1000 AND username <> 'Vancik' AND username <> 'Studentova' AND username <> 'Honza'
;

--  Vancik ADMIN role
INSERT INTO VIZMAN.PERSON_ROLE
	(PERSON_ID, ROLE_ID)
SELECT DISTINCT id, 10001
FROM vizman.person
WHERE username = 'Vancik'
;

--  Studentova MANAGER role
INSERT INTO VIZMAN.PERSON_ROLE
	(PERSON_ID, ROLE_ID)
SELECT DISTINCT id, 10003
FROM vizman.person
WHERE username = 'Studentova'
;
...
-----------------------------------------
--  Salaries - INSERT to PERSON_WAGE
-----------------------------------------

INSERT INTO VIZMAN.PERSON_WAGE
	(ID, VERSION,
	 PERSON_ID,
	 USERNAME,
	 YM_FROM, YM_TO,
	 WAGE)
SELECT USERSAZBA_ID, 1,
	SELECT ID FROM VIZMAN.PERSON pe WHERE LOWER(us.USER_LOGIN) = LOWER(pe.USERNAME),   
	USER_LOGIN,
	ROK_OD * 100 + MES_OD, ROK_DO * 100 + MES_DO,
	USER_SAZBA
FROM ZAVIN.USR_SAZBA us
;

COMMIT;

CREATE UNIQUE INDEX IDXQ_PERSONWAGE_PERSONID_YMFROM
	ON VIZMAN.PERSON_WAGE (PERSON_ID, YM_FROM)
;


-- 15
SELECT COUNT(*) FROM VIZMAN.CALY;
...
-- 7
SELECT COUNT(*) FROM VIZMAN.CFGPROP;

-- 13
SELECT COUNT(*) FROM VIZMAN.CIN;

-- 46
SELECT COUNT(*) FROM VIZMAN.PERSON;

-- 45
SELECT COUNT(*) FROM VIZMAN.PERSON_ROLE;

-- 113
SELECT COUNT(*) FROM VIZMAN.PERSON_WAGE;

-- 3
SELECT COUNT(*) FROM VIZMAN.ROLE;

-- 12
SELECT COUNT(*) FROM VIZMAN.ROLE_PERM;
...
UPDATE ZAVIN.ZAK_
	SET ROKZAK = 2000, ROKMESZAD = '2000-01'
WHERE CISLO_ZAKAZKY = '00001'
;	
...
-- ----------------------------------------------
--   ZAK 00004  U lékaøe  [LEK]
--   ba90ef75-66e6-4fb6-87ca-37c6a41683c8
INSERT INTO VIZMAN.KONT
(ID, VERSION, "UUID", TYP, CKONT, ROK, ARCH, INVESTOR, OBJEDNATEL, MENA, TEXT, FOLDER, ID_KLIENT, TMP, DATE_CREATE, DATETIME_UPDATE)
VALUES(9994, 1, 'ba90ef75-66e6-4fb6-87ca-37c6a41683c8', 'KONT', '00004', 2000, false, NULL, '[ U lékaøe ]', 'CZK', 'U lékaøe', NULL, NULL, NULL, '2000-01-01', '2019-03-29 10:23:54.028')
;
...
-- ----------------------------------------------
--   ZAK 00004  U lékaøe  [LEK]
--   87963a1d-cb7c-40ee-a876-46f13aaa3c83
INSERT INTO VIZMAN.ZAK
(ID, VERSION, "UUID", TYP, CKONT_ORIG, CZAK, ROK, ARCH, SKUPINA, TEXT, FOLDER, HONORAR, ID_KONT, R1, R2, R3, R4, ROZPRAC, R_ZAL, RM, TMP, ROKMESZAD, DATE_CREATE, DATETIME_UPDATE)
VALUES(19994, 1, '87963a1d-cb7c-40ee-a876-46f13aaa3c83', 'LEK', '00004', 1, 2000, false, NULL, 'U lékaøe', NULL, 0.00, NULL, NULL, NULL, NULL, NULL, 0.00, 0, NULL, NULL, '2000-01', '2000-01-01', '2019-03-29 10:28:14.550');
...
UPDATE VIZMAN.ZAK
	SET ID = 19991, TYP = 'REZ'
WHERE CKONT_ORIG = '00001'
;
...
-- 0
SELECT count(*) FROM VIZMAN.ZAK
WHERE ID_KONT IS NULL
;

-- 21312
SELECT MAX(ID) FROM VIZMAN.ZAK
;


-- !!!!!!!!!!!!!!!!!!!!!!!!!!!
--    Nedavat do produkcni !!!
/*
...
-- Empty
SELECT * FROM VIZMAN.DOCH
WHERE PERSON_ID IS NULL
;

-- Make half day dovolena kod 'dp':
UPDATE VIZMAN.DOCH AS doch
...
WHERE CIN_CIN_KOD = 'dp'
;


-- Make full day dovolena kod 'dc':
UPDATE VIZMAN.DOCH AS doch
SET doch.CIN_CIN_KOD = 'dc'
	WHERE DOCH.CIN_CIN_KOD = 'do' AND DOCH.DOCH_STATE IS NOT NULL
;
...
-- 766
SELECT count(*) FROM VIZMAN.DOCH
WHERE CIN_CIN_KOD = 'dc'
;

-- 0
SELECT count(*) FROM VIZMAN.DOCH
WHERE CIN_CIN_KOD = 'do'
;
...
CREATE INDEX VIZMAN.IDX_DOCH_PERSONID_DOCHDATE_FROMTIME ON VIZMAN.DOCH (PERSON_ID, DOCH_DATE, FROM_TIME)
;
...
CREATE UNIQUE INDEX IDXQ_DOCHSUM_PERSONID_DSDATE ON VIZMAN.DOCHSUM (PERSON_ID, DS_DATE);
...
-- =========================================================
--  Update PERSON -> LOGIN_ENABLED and HIDDEN from DOCSUM
-- =========================================================

UPDATE VIZMAN.PERSON
	SET HIDDEN = TRUE
	WHERE ID = 1001 OR ID = 1002 OR ID = 1003 OR USERNAME = 'Honza'
;

-- 14
SELECT DISTINCT person_id
FROM vizman.dochsum
WHERE YEAR(ds_date) = 2018
;

-- Find active users from 2018 and set them not hidden and login enabled
UPDATE VIZMAN.PERSON
	SET HIDDEN = FALSE, LOGIN_ENABLED = TRUE
WHERE ID IN (
	SELECT DISTINCT person_id
	FROM vizman.dochsum
	WHERE YEAR(ds_date) = 2018
...
UPDATE VIZMAN.PERSON
	SET HIDDEN = TRUE, LOGIN_ENABLED = FALSE
WHERE ID NOT IN (
	SELECT DISTINCT person_id
	FROM vizman.dochsum
	WHERE YEAR(ds_date) = 2018
...
CREATE TABLE VIZMAN.DOCHSUM_ZAK (
	...
	CKONT_ORIG VARCHAR(16),
	USERNAME VARCHAR(32),
	DSZ_WORK_PRUH DECIMAL(5,2),
	DSZ_WORK_NORM DOUBLE,
	DSZ_MZDA DECIMAL(8,2),
	DSZ_POJIST DECIMAL(8,2),
	DSZ_MZDAS DECIMAL(8,2),
	DSZ_POJISTS DECIMAL(8,2),
	SAZBA DECIMAL(7,2),
	TMP VARCHAR(1),
...
INSERT INTO VIZMAN.DOCHSUM_ZAK (
	...
	DSZ_WORK_PRUH, DSZ_WORK_NORM,
	DSZ_MZDA, DSZ_POJIST,
	DSZ_MZDAS, DSZ_POJISTS,
	SAZBA
)
SELECT
	VYK_ZAK_ID, 0, 0,
...
-- Lekar - do zakazek - NEBUDEME DAVAT stare !!!
...
-- Empty
SELECT * FROM VIZMAN.DOCHSUM_ZAK
WHERE PERSON_ID IS NULL
;
...
-- Empty
SELECT * FROM VIZMAN.DOCHSUM_ZAK
WHERE ZAK_ID IS NULL
;


-- Kontroly - tabulky ze ZAVINu :
...
-- Kontroly - tabulky z VIZMANA :

-- 1575 (769)...??
SELECT count(*)
FROM vizman.DOCHSUM_ZAK
WHERE DSZ_WORK_NORM = 0
;
...
-- Duplicitni klice:
SELECT ID, CKONT_ORIG, PERSON_ID, DS_DATE
FROM vizman.dochsum_zak
...
-- DROP INDEX VIZMAN.IDXQ_DOCHSUMZAK_PERSON_DATE_ZAK;

CREATE UNIQUE INDEX VIZMAN.IDXQ_DOCHSUMZAK_PERSON_DATE_ZAK ON VIZMAN.DOCHSUM_ZAK (PERSON_ID, DS_DATE, ZAK_ID);
...
DROP TABLE VIZMAN.PRUH IF EXISTS;

CREATE TABLE VIZMAN.PRUH (
	ID BIGINT NOT NULL,
	VERSION INTEGER NOT NULL,
	YM INTEGER NOT NULL,
	PERSON_ID BIGINT NOT NULL,
	STATE INTEGER NOT NULL
			COMMENT '10=OPENED, 20=CLOSED, 30=ANALYSED',
	PERSON_PARAG DECIMAL(6,1)
			COMMENT 'Sum of all non productive activities',
	PERSON_FOND DECIMAL(6,1)
			COMMENT 'Month productivity fond minus non productive hours',
	CONSTRAINT PK_PRUH PRIMARY KEY (ID)
);

CREATE UNIQUE INDEX VIZMAN.IDXQ_PRUH_YM_PERSONID ON VIZMAN.PRUH (YM, PERSON_ID)
;

-- SELECT DISTINCT 100 * ROK_UZAV + MES_UZAV, 0, 100 * ROK_UZAV + MES_UZAV AS YM, SUM(PRACFOND)
SELECT DISTINCT (DS_YM - 200000) * 100 + PERSON_ID AS id, 0 AS version, DS_YM, 30 AS state, SUM (DS_NEM), SUM (DS_DOV), SUM (DS_VOL),
		SUM((CASE WHEN DS_NEM IS NULL THEN 0 ELSE DS_NEM END) +
		    (CASE WHEN DS_DOV IS NULL THEN 0 ELSE DS_DOV END) +
		    (CASE WHEN DS_VOL IS NULL THEN 0 ELSE DS_VOL END)) AS sum
		FROM vizman.DOCHSUM
	GROUP BY id, version, DS_YM, PERSON_ID, state 
--	ORDER BY YM DESC
;

INSERT INTO VIZMAN.PRUH (ID, VERSION, YM, PERSON_ID, STATE, PERSON_PARAG)
SELECT DISTINCT (DS_YM - 200000) * 100 + PERSON_ID AS id, 0 AS version, DS_YM, PERSON_ID, 20 AS state,
		SUM((CASE WHEN DS_NEM IS NULL THEN 0 ELSE DS_NEM END) +
		    (CASE WHEN DS_DOV IS NULL THEN 0 ELSE DS_DOV END) +
		    (CASE WHEN DS_VOL IS NULL THEN 0 ELSE DS_VOL END)) AS sum
		FROM vizman.DOCHSUM
...
----------------------------
--  !!!  NAstavit spravny mesic

UPDATE VIZMAN.PRUH pr
	SET STATE = 10
	WHERE YM = 201903
;
...
SELECT DISTINCT YM, MONTH_FOND_DAYS, MONTH_FOND_HOURS
FROM VIZMAN.CALYM
ORDER BY YM DESC
;

UPDATE VIZMAN.PRUH pr
	SET PERSON_FOND = 0 - PERSON_PARAG + (SELECT MONTH_FOND_HOURS FROM VIZMAN.CALYM ca WHERE ca.YM = pr.YM)
;

SELECT * FROM VIZMAN.PRUH
WHERE PERSON_FOND IS NULL OR PERSON_PARAG IS NULL
;
...
WHERE 
 	CIN_T2 = 'do' OR CIN_T2 = 'dp' OR CIN_T2 = 'dc' OR CIN_T2 = 'ne' OR CIN_T2 = 'nv' OR CIN_T2 = 'L'
...
-- Lekar - do paragrafu takz NEBUDEME DAVAT !!!
-- INSERT INTO VIZMAN.DOCHSUM_PARAG (
-- 	ID,	VERSION, PERSON_ID,
-- 	DS_DATE,
-- 	DS_YM,
