...
CREATE INDEX IDX_KONT_FIRMA ON VIZMAN.KONT (FIRMA);
...
INSERT INTO VIZMAN.KONT (ID, VERSION, CKONT, TEXT, FIRMA, DAT_ZAD, ARCH)
	SELECT VIZMAN.KONT_SEQ.NEXTVAL, 1, CISLO_ZAKAZKY, TEXT, FIRMA, DATUMIMP, ARCH FROM ZAVIN.ZAK_
;
...
CREATE TABLE VIZMAN.ZAK (
	...
	CONSTRAINT FK_ZAK_ZAK FOREIGN KEY (ID_KONT) REFERENCES VIZMAN.KONT(ID) ON DELETE RESTRICT ON UPDATE RESTRICT
);
...
INSERT INTO VIZMAN.ZAK (ID, VERSION, CKONT, CZAK, TYP_DOKLADU,
                        ROKZAK, ROKMESZAD, TEXT, HONORC, ROZPRAC, ARCH,
                        R_ZAL, R1, R2, R3, R4, SKUPINA, RM,
                        DAT_ZAD)
SELECT VIZMAN.ZAK_SEQ.NEXTVAL, 1, CISLO_ZAKAZKY, 1, TYP_DOKLADU,
...
UPDATE VIZMAN.ZAK AS kont
	SET kont.ID_KONT = (SELECT top 1 kont.ID FROM VIZMAN.KONT kont
		WHERE kont.CKONT = kont.CKONT)
	WHERE kont.CKONT IN (SELECT kont.CKONT from VIZMAN.KONT kont where kont.CKONT = kont.CKONT)
;
