...
INSERT INTO VIZMAN.CALYM (ID, VERSION, YM, MONTH_FOND)
...
----------------------------------
-- !!! Find real mmoh fonds !!
----------------------------------
INSERT INTO VIZMAN.CALYM (ID, VERSION, YM, MONTH_FOND)
(
SELECT 100 * YEAR(ymd) + MONTH(ymd), 0,  100 * YEAR(ymd) + MONTH(ymd), fnd
FROM (
	SELECT CONVERT(CONCAT(@yr, '-1-1'), DATE) AS ymd, 444 AS fnd UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-2-1'), DATE), 444 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-3-1'), DATE), 444 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-4-1'), DATE), 444 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-5-1'), DATE), 444 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-6-1'), DATE), 444 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-7-1'), DATE), 444 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-8-1'), DATE), 444 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-9-1'), DATE), 444 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-10-1'), DATE), 444 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-11-1'), DATE), 444 UNION ALL
	SELECT CONVERT(CONCAT(@yr, '-12-1'), DATE), 444 
) yearmonth
)
ORDER BY ymd
;
...
-- !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
--   Ruènì INSERT do KONT 00004  U lékaøe
...
-- !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
--   Ruènì INSERT do ZAK 00004  U lékaøe  [LEK]
...
SELECT DISTINCT (DS_YM - 200000) * 100 + PERSON_ID AS id, 0 AS version, DS_YM, PERSON_ID, 30 AS state,
		SUM((CASE WHEN DS_NEM IS NULL THEN 0 ELSE DS_NEM END) +
		    (CASE WHEN DS_DOV IS NULL THEN 0 ELSE DS_DOV END) +
		    (CASE WHEN DS_VOL IS NULL THEN 0 ELSE DS_VOL END)) AS sum
		FROM vizman.DOCHSUM
...
-- 444 ??
SELECT DISTINCT YM, MONTH_FOND FROM VIZMAN.CALYM
ORDER BY YM DESC
;

UPDATE VIZMAN.PRUH pr
	SET PERSON_FOND = 0 - PERSON_PARAG + (SELECT MONTH_FOND FROM VIZMAN.CALYM ca WHERE ca.YM = pr.YM)
;
